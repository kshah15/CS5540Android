...
public class MainActivity extends AppCompatActivity implements AddToDoFragment.OnDialogCloseListener, UpdateToDoFragment.OnUpdateDialogCloseListener{
    ...
    protected void onStart() {
        ...
        adapter = new ToDoListAdapter(cursor, new ToDoListAdapter.ItemClickListener() {
            // Edit to include parameters for the "category" and "status"
            @Override
    ...
    // ##### inflate the menu for category sorting
    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.main, menu);
        return true;
    }
    // ##### Get the ID of the item that was selected, and update todolist recyclerview to show selected categories
    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            ...
            case R.id.action_dropdown2:
                cursor = getCategoryItems(db, "College");
                adapter.swapCursor(cursor);
                return true;
            ...
            case R.id.action_dropdown4:
                cursor = getCategoryItems(db, "Call");
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_dropdown5:
                cursor = getCategoryItems(db, "Alarm");
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_dropdown6:
                cursor = getCategoryItems(db, "Set Appointment");
                adapter.swapCursor(cursor);
                return true;
            default:
